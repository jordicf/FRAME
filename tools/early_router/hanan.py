...
class HananGrid:
    ...
    def __init__(self, netlist_or_cells: Netlist|list[HananCell]):
        ...
        if isinstance(netlist_or_cells, Netlist):
            ...
            self._shape = Shape(w= float(max(x_coords)-min(x_coords)), h=float(max(y_coords)-min(y_coords)))
    ...
    def get_adjacent_cells(self, cell: HananCell)->list[HananCell]:
        ...
        """
        curr_id = cell._id
        nei = [(curr_id[0] - 1, curr_id[1]), (curr_id[0] + 1, curr_id[1]), (curr_id[0], curr_id[1] - 1), (curr_id[0], curr_id[1] + 1)]
        adjacent_cells = []
        for n in nei:
        ...
        for cell in hanan_grid.cells:
            ...
            for l_id in self._layers:
                ...
                if l_id != 0:
                    ...
                    self._adj_list.setdefault(node_id, {})[node_below] = self.add_edge3D(node_id, node_below)
                    self._adj_list.setdefault(node_below, {})[node_id] = self.add_edge3D(node_below, node_id)
    ...
    def _add_terminals(self, hanan_grid:HananGrid, netlist:Netlist):
        ...
        for m in netlist.modules:
            if m.is_terminal:
                ...
                terminal_id = (t,-1, 0) # Terminals are always on the lowest layer
