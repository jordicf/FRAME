...
def run_analyze(options: dict[str, Any]) -> None:
    ...
    if analyze_type == "hyperparams":
        ...
        if options['draw_congestion']:
            ...
            congest_map = draw_congestion(netlist, ft1.solution.congestion, ft1.hanan_graph)
            congest_map.save(f"{output_path}/{file_path.stem}map_HC.gif", quality=95)

            congest_map = draw_congestion(netlist, ft2.solution.congestion, ft2.hanan_graph)
...
def low_congestion_opt(file_path, options, low = 0., high=152.0, eps=0.01, max_iter=100):
    # Binary search
    sol = None
    itr =0
    ...
    if best_c is not None and sol is not None:
        print(f"NO congestion with pitch: {best_c}, and number of layers: {math.ceil(76/best_c)}")
...
def high_congestion_opt(file_path, options, low = 0., high=152.0, eps=0.01, max_iter=100):
    # Binary search
    sol = None
    itr =0
    # Binary search for the high-congestion
    ...
    if best_c is not None and sol is not None:
        sol._m['opt_nlayers'] = math.ceil(76/best_c)
        print(f"Best working pitch found: {best_c}, with opt number of layers: {math.ceil(76/best_c)}")
        return sol, best_c
    else:
        print("No suitable pitch found.")
        return None
...
def hyperparameter_analysis(ft:FeedThrough, output:Path, filename:str):
    ...
    for _ in range(num_initial):
        while True:
            ...
            if alpha >= min_weight and beta >= min_weight and gamma >= min_weight: # Avoiding numerical inestability
                break
    ...
    for _ in range(40):  # total 50 trials
        ...
        while True:
            ...
            if new_alpha >= min_weight and new_beta >= min_weight and new_gamma >= min_weight:# Avoiding numerical inestability
                break
