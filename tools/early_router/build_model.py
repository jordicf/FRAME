...
class FeedThrough:
    ...
    class VariableStore:
        ...
        def get_unrouted(
            self, result: SolveResult, nets: dict[NetId, NamedHyperEdge | HyperEdge]
        ) -> list[NetId]:
            """Update the routes and congestion properties.
            Useful after solving the problem."""
            vals = result.variable_values()  # Dict var:value
    ...
    def _find_opt_bb(
        self, unrouted_nets: list[NetId] = None, depth: int = 0, max_depth: int = 3
    ):
    ...
    def solve(
        ...
        module_crossings = self._variables.get_variables(crossing=True)
        wire_length = [
            var * self._graph.get_edge(var_id[0], var_id[1]).length
            for var_id, var, _ in self._variables
        ]
        via_usage = self._variables.get_variables(via=True)
