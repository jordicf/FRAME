...
from tools.early_router.hanan import (
    HananGrid,
    HananEdge3D,
    HananGraph3D,
    Layer,
...
class FeedThrough:
    ...
    class VariableStore:
        ...
        def add(
            ...
            self._vars.setdefault((var_id[0], var_id[1]), {})[var_id[2]] = (
                var,
                (crossing, via, source),
            )
        ...
        def update(self, result: SolveResult):
            ...
            for var_id, var, flags in iter(self):
                ...
                if wire > self._tol:
                    ...
                    self._routes.setdefault(net_id, []).append({edge_id: wire})
                    reverse_key = edge_id[::-1]
                    self._congestion[edge_id] = (
                        self._congestion.get(edge_id, 0)
                        + wire
                        + self._congestion.pop(reverse_key, 0)
                    )

        def get_unrouted(
            self, result: SolveResult, nets: dict[NetId, NamedHyperEdge]
        ) -> list[NetId]:
            """Update the routes and congestion properties.
            Useful after solving the problem."""
            vals = result.variable_values()  # Dict var:value
    ...
    def add_nets(
        ...
        for net_id, net in it:
            ...
            if net.weight < 1:
                warnings.warn(
                    f"Converting netid ({net_id}) weight lower than 1 to an int!",
                    UserWarning,
                )
                wei = net.weight
            else:
    ...
    def _add_2pin_net(self, net_id: int, 
        ...
        for e_id in net_edges:
            ...
            var: Variable = self._solver.add_variable(
                lb=0, ub=int(wei) + 1, name=f"x_{e.source._id}_{e.target._id}_{net_id}"
            )
            ...
            if (
                e.crossing
                and e.source.modulename not in modulenames
                and e.target.modulename not in modulenames
            ):
            ...
            self._variables.add(
                (e.source._id, e.target._id, net_id),
                var,
        ...
        for n in net_nodes:
            ...
            in_var = [
                ...
                for e in in_out_edges["in"]
                if (e.source._id, e.target._id) in net_edges
            ]
            out_var = [
                net_variables[(e.source._id, e.target._id, net_id)]
                for e in in_out_edges["out"]
                if (e.source._id, e.target._id) in net_edges
            ]
        ...
        for source in self._graph.adjacent_list:
            ...
            for target in self._graph.adjacent_list[source]:
                ...
                var: Variable = self._solver.add_variable(
                    ...
                    name=f"x_{source}_{target}_{net_id}",
                )
    ...
    def _add_sub2pin_net(self, net_id: int, subnet: NamedHyperEdge, subnet_id: str, nodes: list[HananNode3D],
        ...
        for e_id in edgeids:
            var: Variable = self._solver.add_variable(
                lb=0,
                ub=int(subnet.weight) + 1,
                name=f"{subnet_id}_{e_id[0]}_{e_id[1]}_{net_id}",
            )
    ...
    def _compute_norms(self):
        ...
        for n in self._nets.values():
            centers_x = {
                c.center.x
                for m in n.modules
                for c in self._graph.get_nodes_by_modulename(m)
            }  # Should compute the centroid and not the cell
            centers_y = {
                c.center.y
                for m in n.modules
                for c in self._graph.get_nodes_by_modulename(m)
        ...
        if hasattr(self, "pre_result"):
            ...
            wl = [
                self.pre_result.variable_values(var)
                * self._graph.get_edge(var_id[0], var_id[1]).length
    ...
    def _find_opt_bb(self, unrouted_nets: list[NetId] | None = None, depth: int = 0, max_depth: int = 3, verbose:bool=True) -> bool:
        ...
        if unrouted_nets is None:  # Solving call
            ...
            self._m["#unrouted_nets"] = (
                str(self._m.get("#unrouted_nets", "")) + f"{len(unrouted)} - "
            )
            if len(unrouted) < 6:
                self._m["unrouted_nets_ids"] = (
    ...
    def solve(self, f_wl: float = 0.1, f_mc: float = 0.2, f_vu: float = 0.7, 
        ...
        self._m["factors"] = (
            f"wl={round(f_wl, 3)} mc={round(f_mc, 3)} vu={round(f_vu, 3)}"
        )
    ...
    def _metrics(self):
        ...
        if self.is_solved:
            wire_length = [
                self.result.variable_values(var)
                * self._graph.get_edge(var_id[0], var_id[1]).length
                for var_id, var, _ in self.solution
            ]
            self._m["total_wl"] = round(sum(wire_length), 0)
            self._m["module_crossings"] = round(
                sum(
                    self.result.variable_values(
                        self.solution.get_variables(crossing=True)
                    )
                ),
                1,
            )
    ...
    def has_bifurcation(
        self, net: NamedHyperEdge, route: list[dict[EdgeId, float]]
    ) -> bool:
        """
        ...
        nodes_set = [
            set([n._id for n in self._graph.get_nodes_by_modulename(m)])
    ...
    def get_route_path(
        self, identifiers: list[dict[EdgeId, float]]
    ) -> list[dict[EdgeId, float]] | None:
    ...
    def _congestion_map(
        self, layer_id: list[int] = []
    ) -> dict[tuple[CellId, CellId], tuple[float, float]]:
    ...
    def check_integral_solution(
        self, tol: float = 1e-9
    ) -> Tuple[bool, List[Tuple[NetId, EdgeId, float]]]:
        """
