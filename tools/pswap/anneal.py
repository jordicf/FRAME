...
def simulated_annealing(
    net: swapNetlist, n_swaps: int, patience: int,
    ...
    # Initial solution
    x_best = [p.x for p in net.points]
    y_best = [p.y for p in net.points]
    ...
    while no_improvement < patience:
        ...
        iter += 1
        if verbose:
            print(f"Iter. {iter}, Temp. {temp:.3f}, HPWL: Avg {avg:.1f}, Best Avg {best_avg:.1f}, Best {best_hpwl:.1f}"
            )
...
def _find_best_temperature(net: swapNetlist, nswaps: int,
                           target_acceptance: float) -> float:
    ...
    init_xy = [(p.x, p.y) for p in net.points]
    cost = []
    for _ in range(nswaps):
        idx1, idx2 = np.random.choice(net.movable, 2, replace=False)
        delta_hpwl = net.swap_points(idx1, idx2)
        cost.append(abs(delta_hpwl))
    # Recover original positions and hpwl
    for i, p in enumerate(net.points):
        p.x, p.y = init_xy[i][0], init_xy[i][1]
    net.hpwl = sum(net.compute_net_hpwl(n) for n in net.nets)
