...
class TestSAT(unittest.TestCase):
    ...
    def test_sudoku(self):
        ...
        for i in range(0, size):
            for j in range(0, size):
                for k in cord:
                    ...
                    exactlyOne( sm, [ table[size*i + k1][size*j + k2][k] for k2 in range(0, size) for k1 in range(0, size) ] )
        ...
        for i in range(0, size):
            for j in range(0, size):
                # Numbers do not repeat on each quadrant
                self.assertTrue(allDifferent( [ sol[size*i + k1][size*j + k2] for k2 in range(0, size) for k1 in range(0, size) ] ))
