...
class ModelWrapper:
    ...
    def add_macro(self, macro):
        if macro.is_terminal:
            self.terminal_macros.add(id(macro))
        self.macros.append(macro)

    def add_coordinates(self, x: ExpressionTree, y: ExpressionTree, w: ExpressionTree, h: ExpressionTree):
        if not any(m for m in self.macros if id(m) in self.terminal_macros 
                and m.x[0] == x and m.y[0] == y):
    ...
    def dif_cost_objective(self):
        ...
        for var in self.variable_list:
            delta = self.gekko.Intermediate(var - var.evaluate())  
            squared = self.gekko.Intermediate(delta**2)            
            smoothed = self.gekko.Intermediate(squared + 0.0001)  
            obj += self.gekko.sqrt(smoothed)                       
    ...
    def set_objective_function(self, objective: ExpressionTree):
        ...
        for var in objective.get_variable_list():
            # dealing with var.data is None
            if var.data is None:
                var_name = getattr(var.value, 'name', f'_auto_{id(var.value)}')
    ...
    def build_model(self, small_steps=False, radius=None):
        ...
        self.gekko.solver_options = ['minlp_gap_tol 1.0e-2',\
        ...
        self.gekko.options.SCALING=1
        self.gekko.MEAS_CHK = 0
        set_epsilon_gekko(self.gekko)
        self.macro_constraints = dict()
        for macro in [m for m in self.macros if not m.is_terminal]:
