...
class Allocation:
    ...
    def __init__(self, stream: Python_object | str):
        """
        Creates an allocation of rectangles from a Python object or from a YAML
        specification
        :param stream: a Python object, or a YAML specification (text).
        """
        if isinstance(stream, str):
            stream = read_json_yaml_text(stream, False)
        self._parse_yaml_tree(stream)
    ...
    def initial_allocation(
        self, netlist: Netlist, include_area_zero: bool = False
    ) -> "Allocation":
        """
        ...
        for a in self.allocations:
            ...
            for m in netlist.modules:
                area = sum(
                    a.rect.area_overlap(r_mod) / a.rect.area for r_mod in m.rectangles
                )
    ...
    def refine(self, threshold: float, levels: int = 1) -> "Allocation":
        ...
        for a in self.allocations:
            ...
            new_alloc.extend(
                self._split_allocation(a.rect, a.alloc, a.depth, levels if split else 0)
            )
        return Allocation(new_alloc)
    ...
    def must_be_refined(self, threshold: float) -> bool:
        ...
        """
        return any(
            all(x <= threshold for x in a.alloc.values()) for a in self.allocations
        )
            ...
            for _ in range(n):
                ...
                if not a.rect.fixed and a.rect.x_cuttable(x_cuts[i], 0.01):
                    ...
                    for rect in [r1, r2]:
                        new_allocs.append(
                            RectAlloc(
                                rect, {m: r for m, r in a.alloc.items()}, a.depth + 1
                            )
                        )
                else:
                    new_allocs.append(a)
    ...
    def uniform_refinement_depth(self) -> "Allocation":
        ...
        for a in self.allocations:
            new_alloc.extend(
                self._split_allocation(a.rect, a.alloc, a.depth, max_depth - a.depth)
            )
        return Allocation(new_alloc)
    ...
    def write_yaml(self, filename: str | None = None) -> None | str:
        ...
        """
        list_modules = [
            [r.rect.vector_spec, r.alloc, r.depth]
            if r.depth > 0
    ...
    def _parse_yaml_tree(self, tree: Python_object) -> None:
        ...
        self._allocations = list[RectAlloc]()
        self._module2rect = dict[str, list[ModuleAlloc]]()
        for i, alloc in enumerate(tree):
            # Read one rectangle allocation

            if isinstance(alloc, list):
                alloc = tuple(alloc)
            assert isinstance(alloc, tuple) and 2 <= len(alloc) <= 3, (
                f"Incorrect format for rectangle {alloc}"
            )
            ...
            for module, occup in allocs.items():
                ...
                if module not in self._module2rect:
                    self._module2rect[module] = list[ModuleAlloc]()
    ...
    def _split_allocation(
        rect: Rectangle, alloc: Alloc, depth: int, levels: int = 0
...
def create_initial_allocation(die: Die, include_area_zero: bool = False) \
    -> Allocation:
    """
    ...
    return Allocation(allocation_list).initial_allocation(
        die.netlist, include_area_zero
    )
