...
# Descriptor of a rectangle: ((x, y, w, h), alloc, depth)
RectDescriptor = tuple[tuple[float, float, float, float], Alloc, int]
...
class Allocation:
    ...
    @staticmethod
    def _split_allocation(rect: tuple[float, float, float, float], alloc: Alloc, depth: int, levels: int = 0) \
            -> list[RectDescriptor]:
        """
