...
from frame.utils.utils import is_number, string_is_number, valid_identifier, read_json_yaml, TextIO_String
...
def parse_yaml_die(stream: TextIO_String) -> tuple[Rectangle, list[Rectangle]]:
    ...
    for key in tree:
        assert key in [KW_WIDTH, KW_HEIGHT,
                       KW_REGIONS], f"Unknown keyword in die: {key}"
    ...
    assert is_number(
        shape.w) and shape.w > 0, "Wrong specification of the die width"
    assert is_number(
        shape.h) and shape.h > 0, "Wrong specification of the die height"
    die = Rectangle(
        **{KW_CENTER: Point(shape.w/2, shape.h/2), KW_SHAPE: shape})
    ...
    if KW_REGIONS in tree:
        ...
        assert isinstance(rlist, list) and len(
            rlist) > 0, f"Incorrect specification of die rectangles"
...
def parse_die_rectangle(r: list):
    ...
    :return: a Rectangle
    """
    assert isinstance(r, list) and len(
        r) == 5, "Incorrect format of die rectangle"
    for i in range(4):
        assert is_number(
